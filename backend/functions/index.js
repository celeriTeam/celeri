        ...
        .then(async (duelsSnapshot) => {
          ...
          for (const duelDoc of duelsSnapshot.docs) {
            ...
            try {
              ...
              if (duelData.bets && Array.isArray(duelData.bets)) {
                // Calculate total wagers and total wagers on winner
                duelData.bets.forEach((bet) => {
                  totalWagers += bet.wager;
                  if (bet.betOnUserID === winner) {
                    totalWagersOnWinner += bet.wager;
                  }
                });
                ...
                duelData.bets.forEach((bet) => {
                  ...
                  if (bet.betOnUserID === winner) {
                    percentage = bet.wager / totalWagersOnWinner;
                  ...
                  if (bet.betOnUserID === winner) {
                    batch.update(userRef, {
                      tokens: firestore.FieldValue.increment(amountWon),
                    });
                  } else {
                    batch.update(userRef, {
                      tokens: firestore.FieldValue.decrement(bet.wager),
                    });
                  }

                  // Update totalWinnings in the duel document
                  batch.update(duelDoc.ref, {
                    totalWinnings: admin.firestore.FieldValue.arrayUnion({
                      userID: bet.userID,
                      amountWon: amountWon,
                    }),
                  });
              ...
              // Mark duel as ended and set winner
              batch.update(duelDoc.ref, {ended: true, winner: winner});
            } catch (error) {
              console.error(`Error fetching player steps for duel ${duelDoc.id}:`, error);
            }
          }

          // After processing all duels for the group, commit the batch
